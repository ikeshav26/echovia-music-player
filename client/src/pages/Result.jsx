...
import { Music, Play, Pause, Plus, ArrowLeft, Search } from "lucide-react";
...
const Result = () => {
  ...
  const fetchAllSongs = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/songs/all-songs`, { 
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="mr-2 lg:ml-24 lg:mr-96 pt-20 px-2 md:px-6 h-screen">
        <div className=" rounded-2xl p-4 md:p-8 relative z-0 h-[calc(100vh-13rem)] lg:h-[calc(100vh-10.5rem)] overflow-y-auto scrollbar-thin scrollbar-track-white/5 scrollbar-thumb-white/20 hover:scrollbar-thumb-white/30">
          ...
          <div className="mb-6 md:mb-8">
            <div className="flex items-center gap-3 md:gap-4 mb-4">
              ...
              <div>
                <h1 className="text-2xl md:text-4xl font-bold mb-1 md:mb-2">Search Results</h1>
                <p className="text-sm md:text-base text-gray-400">
                  {searchQuery ? `Results for "${searchQuery}"` : 'No search query'}
                </p>
              </div>
            </div>
          </div>
