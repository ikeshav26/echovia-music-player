...
const Signup = () => {
  ...
  const submithandler=async(e)=>{ 
    ...
    try{
      const res=await axios.post(`${import.meta.env.VITE_API_URL}/users/signup`, userData,{withCredentials: true});
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 overflow-hidden">
      ...
      <div className="relative z-10 bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-8 border border-white/20 hover:bg-white/15 transition-all duration-300 hover:scale-105 hover:shadow-purple-500/20">
        ...
        <div className="flex items-center my-6">
          ...
          <span className="px-4 pb-1 text-gray-300 text-sm bg-white/10 rounded-full backdrop-blur-sm">or</span>
          <div className="flex-grow h-px bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
        </div>

        <p className="text-center text-gray-300 text-sm mt-6">
          Already have an account?{" "}
          <Link to={"/login"} className="text-purple-400 hover:text-purple-300 hover:underline transition-colors duration-300 font-medium">
            Log in
          </Link>
        </p>
      </div>
